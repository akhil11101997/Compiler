...
struct
  ...
  fun add s (Ast.NIL ) = "nil"
    ...
    |add s (Ast.Closed x) = (addlist (s+1)x)^(put s)^")"
    ...
    |add s (Ast.LET(a, b) ) = (put s)^blue^"let"^white^(adddeclist (s+1)a)^blue^(put s)^"in"^white^(addlist (s+1)b)^(put s)^blue^"end"^white
   				
and
      adding s (Ast.VariableDec a) = (pvardec s a)
      |adding s (Ast.TypeDec (a, b) ) = (put s)^"type "^a^" = "^(printty 0  b)
      |adding s (Ast.ClassDec (a,b) ) = (put s)^"class "^a^(addclasslist (s+1)b)^(put s)
...
and
	...
	|printty s (Ast.ClassDefCan a)	= "class { "^(addclasslist (s+1)a)^(put s)^"}"
	|printty s (Ast.ClassDefCanType (a,b)) = "class  extends "^a^"{ "^(addclasslist (s+1)b)^(put s)^"}"
and
      adddeclist s []      = ""
     |adddeclist s (x::xs) = (adding s x)^(adddeclist s xs)
and 
      addlist s []      = " "
     |addlist s (x::[]) = (add s x)
...
and
	addclasslist s []      = ""
     |addclasslist s (x::xs) = (classfield s x)^(addclasslist s xs)
and
