(* here we will find nullable sysmbols and then we will find first and follow *)
use "overview_of_grammer.sml";
use "grammar.sml";

(* AtomSet to store nullable symbols *)
...
(*here we intialize map for all symbols and this we will use to find first and follow *)
fun init map_set = let fun insert_map_symbol (x::xs) = (map_set := AtomMap.insert (!map_set, x, ref AtomSet.empty); insert_map_symbol xs)
...
fun in_atom_list lst x = let fun cmp y = Atom.compare (x, y) = EQUAL in
                            		List.exists cmp lst
                        		end
...
fun check_nullable_symbols (x::xs) = (if in_atom_list (!nullable) x then () 
...
fun find_first_prod y (x::xs) = (add_first_symbol y x; if in_atom_list (!nullable) x then find_first_prod y xs else ())
|	find_first_prod _ _ = ();
...
fun add_follow_symbol y x (xs::xss) = (let val fst_xs = if AtomSet.member (#tokens grammar, xs) then AtomSet.add (AtomSet.empty, xs)
										...
										if in_atom_list (!nullable) xs then add_follow_symbol y x xss else ())
